...
<script lang="ts">
	...
	$: {
		fields = fields.map((f) => {
			let properties = f.field?.geojson?.properties ? f.field.geojson.properties : {};
...
<div class="m-2">
	...
	{:else}
		<h1>One more step</h1>
		<p>
			Shapefiles are great, but do require a bit of human interpretation. Can you tell me which
			column is the field name?
		</p>
		<label>
		...
		<div class="card-grid">
			{#each fields as field}
				<label class="floating relative">
					<input type="checkbox" on:click={() => (field.checked = !field.checked)} />
					<CheckmarkIcon
